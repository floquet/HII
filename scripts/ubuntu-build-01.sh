...
# mkdir topa; mkdir apt-get; cd apt-get/ ; apt-get update ; apt-get install -y vim
# select 2, 49
# cp /dantopa/repos-xiuhcoatl/github/hii/scripts/ubuntu-build-01.sh .
# chmod +x ubuntu-build-01.sh
# sudo ./ubuntu-build-01.sh 2>&1 | tee ubuntu-build.log
...
new_step "Creating logging directory structure"
sub_step "Creating ./apt-get directory"
    mkdir -p ./apt-get
sub_step "Creating log files"
    touch ./apt-get/success.txt
    touch ./apt-get/failure.txt
sub_step "Setting up logging variables"
    SUCCESS_LOG="./apt-get/success.txt"
    FAILURE_LOG="./apt-get/failure.txt"
...
sub_step "Running apt-get upgrade"
    apt-get upgrade -y > "./apt-get/upgrade.log" 2>&1
